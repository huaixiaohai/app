 ...
 */
 ...
 import { ElMessage, ElAlert } from 'element-plus' // UI组件库
        ...
        if (res.status !== 200) {
            ...
            if (res.status === 401 || res.status === 403 || res.status === 408) {
                ...
                (ElAlert as any).warning({
                    ...
                    onOk: () => {
                        setTimeout(() => {
                            (ElAlert as any).remove();
                            (ElAlert as any).info('退出成功');
                            // 可在此处清空token等操作
...
export const request = (params: { url: string; method: any; data: any; }) => {
    return service({
        url:params.url,
